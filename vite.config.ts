...
function cspQuotesProxy(quotesProxy: string | undefined) {
...
export default defineConfig(({ mode }) => {
  // Read config through loadEnv, NOT process.env. Vite loads .env files into the
  // app's `import.meta.env`, but the config module never sees them. Reading
  // process.env here left the CSP and the app disagreeing whenever the proxy
  // came from .env.local: the app called the worker while the CSP still
  // allow-listed only the public proxies, so the browser blocked every quote.
  // loadEnv covers both sources — .env files AND real env vars (as CI sets).
  const env = { ...loadEnv(mode, process.cwd(), 'VITE_'), ...process.env }
  const quotesProxy = env.VITE_QUOTES_PROXY
